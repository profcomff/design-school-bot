...
class Workflow:
    ...
    def start_message(cls, direction_id: int):
        return f"""
        ARE YOU READYYYY?? Мы уже держим палец над кнопкой, которая откроет тебе доступ к курсу '{get_directions()[direction_id - 1].name}'.
        ...
        Ссылка на чат: {get_directions()[direction_id - 1].chat_ref}
        """
    ...
    @classmethod
    def end_course_message(cls, direction_id: int):
        return f"""Мы тоже хотим стать больше и круче, и нам будет очень приятно, если ты дашь небольшой фидбек по ссылке: {get_directions()[direction_id-1].end_link}"""
